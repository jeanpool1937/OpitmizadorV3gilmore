...
// Sub-component: Date Compliance Table
const DateComplianceView: React.FC<{
    schedule: DailyPlan[],
    demands: Demand[]
}> = ({ schedule, demands }) => {
                        ...
                        {comparisonData.map((row, idx) => (
                            <tr key={idx} className="hover:bg-slate-50">
                                ...
                                <td className="px-6 py-3 text-slate-600">
                                    <div>{formatDate(row.demandDate)}</div>
                                    <div className="text-xs font-bold text-blue-700 mt-0.5">{row.toMake.toFixed(1)} T</div>
                                </td>
...
// Sub-component: Calendar Grid View
const CalendarGridView: React.FC<{ schedule: DailyPlan[] }> = ({ schedule }) => {
...
export const Results: React.FC<ResultsProps> = ({ batchResult, allDemands, config, onReoptimizeCoil, onGenerateSchedule, onSmartSchedule }) => {
    ...
    const totalGlobalPatterns = useMemo(() => {
        return Object.values(batchResult.results).reduce((acc: number, res: any) => acc + res.patterns.length, 0);
    }, [batchResult]);
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
                ...
                <div className="flex gap-2">
                    {/* Sequencing Button (Unified) */}
                    {(onSmartSchedule || onGenerateSchedule) && (
                        <button
                            onClick={onSmartSchedule || onGenerateSchedule}
                            className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors shadow-sm mr-2"
                            title="Generar programación optimizando fechas para llegar 2 días antes (JIT)"
                        >
                            <CalendarClock className="w-4 h-4" />
                            Programar Inteligente (JIT)
                        </button>
                    )}
                                ...
                                {batchResult.globalCapacitySchedule.map((day, idx) => (
                                    <div key={idx} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                                        <div className="bg-slate-50 px-6 py-3 border-b border-slate-200 flex justify-between items-center">
                                            ...
                                            <div className="flex items-center gap-4">
                                                {/* Daily Yield Badge */}
                                                {day.capacityUsedPercent >= 99 && (
                                                    <span className="text-[10px] font-bold text-white bg-indigo-600 px-2 py-1 rounded border border-indigo-700 shadow-sm animate-pulse mr-2">
                                                        ★ 100% CAP
                                                    </span>
                                                )}
