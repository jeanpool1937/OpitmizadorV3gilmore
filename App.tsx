...
import { solveBatchCuttingStock, prepareCoilGroups, solveLinearHybrid, calculateGlobalSchedule, calculateMaxLateness, solveBatchCuttingStockBestOf } from './utils/solver';
...
const App: React.FC = () => {
  ...
  const handleGenerateSchedule = () => {
      ...
      setTimeout(() => {
          try {
            ...
            const schedule = calculateGlobalSchedule(
  ...
  const handleSmartScheduleDate = () => {
     if (!multiCoilResult || multiCoilResult.globalCapacitySchedule.length === 0) {
         alert("Primero debes generar una secuencia para poder optimizar la fecha.");
         return;
     }
     ...
     setTimeout(() => {
         try {
             ...
             const activeDemands = demands.filter(d => d.coilCode !== RAPID_COIL_CODE);
             const latenessDays = calculateMaxLateness(multiCoilResult.globalCapacitySchedule, activeDemands);

             if (latenessDays === 0) {
                 alert("El programa ya está perfectamente sincronizado o no hay demanda pendiente.");
                 setIsSolving(false);
                 return;
             }
             ...
             // Actually, lateness is (Prod - Demand). If Prod > Demand, diff is + (Late). We need to start Earlier.
             // So NewStart = Current - Diff.
             // Example: Late by 5 days. Start = Current - 5.
             // Example: Early by 10 days (Diff = -10). Start = Current - (-10) = Current + 10.
