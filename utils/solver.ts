...
const runLP = (
    ...
    demands.forEach(d => {
        const minVal = Math.max(0, d.targetTons * (1 - effectiveTolerance / 100));
        const maxVal = d.targetTons * (1 + effectiveTolerance / 100);
...
const solveLinearPhase = (
    ...
    const effectiveTolerance = overrideTolerance !== undefined ? overrideTolerance : config.tolerance;
    ...
    // 3. Solve Initial LP
    const lpResult = runLP(allGeneratedPatterns, aggregatedDemands, config, effectiveTolerance);
...
export const solveLinearHybrid = (demands: Demand[], config: SolverConfig, minimizePatterns: boolean = false): OptimizationResult => {
    ...
    while (improvementFound && iterations < MAX_ITERATIONS) {
        ...
        const resTest = runLP(testSet, aggregatedDemands, config, config.tolerance);
...
export const calculateGlobalSchedule = (
    ...
    while (queueIndex < queue.length && dayCount < MAX_DAYS) {
        ...
        // Calculate visuals (Use the specific day capacity for percentage)
        const finalEffectiveCapacity = Math.max(0.1, currentDailyCapacity); 
        dayPlan.capacityUsedPercent = (dayPlan.totalTons / finalEffectiveCapacity) * 100;
...
export const prepareCoilGroups = (allDemands: Demand[]): CoilGroupConfig[] => {
    ...
    allDemands.forEach(d => {
        ...
        if (!groups[code]) {
            ...
            // 2. Check Master List (Exact Override)
            if (COIL_WIDTH_MASTER_LIST[code]) {
                detectedWidth = COIL_WIDTH_MASTER_LIST[code];
            } else {
